...
| Embedding | **Voyage AI (voyage-4, 1024 boyut)** | Anthropic'in Claude ile kullanım için resmi önerdiği sağlayıcı — ilk 200M token ücretsiz, ön ödeme istemiyor (hackathon için pratikte sıfır maliyet). |
| LLM (Keşif/Tanımlama, Faz 1) | **Gemini 3.6 Flash (ücretsiz katman)** | Bütçe kısıtı: Anthropic ön ödeme istiyor, Gemini istemiyor. (Önce 2.5 Flash seçilmişti; Google o modeli yeni kullanıcılara kapattı ve yerine 3.6 Flash'ı önerdi.) Ücretsiz katmanda günlük kota model başına ayrı işlediği için ajan, ana model kullanılamadığında LangChain'in `with_fallbacks` mekanizmasıyla aynı prompt ve şemayı Gemini 3.1 Flash-Lite'a gönderir. **Yedeğe düşüren iki durum var:** kota dolması (`GoogleRateLimitError`, 429/RESOURCE_EXHAUSTED) ve sağlayıcı kaynaklı geçici arıza (`GoogleAPIError`; kütüphane bu sınıfı yalnızca 5xx için üretiyor — örn. "503 UNAVAILABLE: This model is currently experiencing high demand"). 503 sonradan eklendi: gerçek bir denemede kanıt değerlendirme isteği bu yüzden düştü. Yoğunluk da kota gibi model başına oluştuğu için yedek model o anda genellikle boşta. **4xx hataları listede yok:** hatalı bir istek ikinci modelde de aynı şekilde başarısız olur, ikinci çağrı israf olurdu. İki model de yanıt veremezse istek 503 olarak dönüyor (kota için 429) — ayrıntı `app/core/hatalar.py`. Sağlayıcı SDK'sının kendi tekrar sayısı 6'dan **2'ye** indirildi (`MAKS_TEKRAR`): üstel beklemeyle yedek modele geçiş dakikalara çıkıyordu ve yoğun olan modeli tekrar tekrar denemenin faydası yok — asıl tekrar stratejimiz yedek model. Her çağrının süresi ve kaçıncı deneme olduğu sunucu günlüğüne yazılıyor. LangChain soyutlaması sayesinde tek satırlık bir değişiklik — ücretli bir sağlayıcıya geçiş her zaman mümkün. Gerçek jüri demosuna yaklaşınca (hız sınırı ve veri kullanımı riskleri nedeniyle) yeniden değerlendirilecek. |
| LLM (Eşleştirme/Doğrulama, Faz 2) | Henüz kesinleşmedi | Faz 2'ye gelince, o zamanki bütçe durumuna göre karar verilecek. |
